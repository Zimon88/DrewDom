...
public class ZamowieniePozycja extends SQLiteConnection{
    ...
    public ArrayList<ZamowieniePozycja> getPozycjeZamowienia2(String numer){
        ArrayList<ZamowieniePozycja> lista = new ArrayList();
        String sql = "SELECT * FROM "+ TABLE_NAME +" WHERE "+COL_NRZAMOWIENIA+" = '"+numer+"' AND id NOT IN (SELECT pozycja FROM priorytety);";
        connect();
        try {
            ResultSet w = stmt.executeQuery(sql);
            int wynik =0;
